...
local StarServer = Star:Server("Real Hub",5012540623) --ใส่idรูปได้
...
AutoFarm:Toggle("Auto Farm Superhuman", _G.FullyAutoSuperhuman, function(vu)
...
AutoFarm:Toggle("Auto Farm Mastery Devil Fruit", false, function(vu)
	...
	if AutoFarmMasteryFruit and WeaponMastery == "" then
		DiscordLib:Notification("AutoFarm Mastery","SelectWeapon First","Okay")
	else
		FarmMasteryFruit = vu
		if vu == false then
			wait(1)
...
AutoFarm:Toggle("AutoFarm Mastery Gun", false, function(vu)
...
HealthPersen = 25
AutoFarm:Slider("Set Health", 1,500,HealthPersen, function(Value)
	HealthPersen = Value
end)
...
Stats:Toggle("Fruit", _G.DevilFruit, function(vu)
    DevilFruit = vu
end)
...
Players:Label("Not Support Auto Farm Players")

Player = ""
local SelectPlayer = Players:Dropdown("Select Player", PlayerName, function(vu)
...
Players:Toggle("ESP Player",false,function(Value)
    PlayerESP = Value
...
local Devil = StarServer:Channel("Fruits")

Devil:Toggle("ESP Fruit",false,function(Value)
    DevilESP = Value
...
Devil:Toggle("Auto Random Fruit", false, function(v)
    DevilAutoBuy = v
end)
...
local BuyItem = StarServer:Channel("Item")
...
Setting:Toggle("HitBox", true, function(t)
    HideHit = t
end)

Setting:Line()

Setting:Button("Remove Lava", function()
